...
query.dropna()
query2 = df[['occupation']].groupby(['occupation']).size().reset_index(name='counts')
#print(query)
# 1 => 1 : 26
start = 0
end = 27
...
x = np.arange(len(query2['occupation']))  # the label locations
...
ax.set_title('Class Percentage of occupation')
ax.set_xticks(x)
